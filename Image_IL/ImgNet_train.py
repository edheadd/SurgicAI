...
data_dir = str(_IL_DATA_DIR / task_name) if _IL_DATA_DIR is not None else str(_REPO_ROOT / "Image_IL" / "SRC_img_data" / task_name)
model_save_dir = str(_IL_OUT_DIR / task_name / f"Imgnet_{view_name}_view" / "Model")
...
if args.use_wandb:
    wandb.init(project=args.wandb_project, name=(args.wandb_name or f"Imgnet_{task_name}_{view_name}_view"))
