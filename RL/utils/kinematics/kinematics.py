...
PSM_TYPE_DEFAULT = PSMType.LND_SI.value
TOOL_TYPE_DEFAULT = ToolType.LND_SI.value
...
class PSMKinematicSolver:
    def __init__(self, root_dir=config_folder, psm_type=PSM_TYPE_DEFAULT, tool_id=TOOL_TYPE_DEFAULT):
        ...
        self.JOINT_LIMITS_LOWER = np.array(self.lower_limits, dtype=np.float32)
        self.JOINT_LIMITS_UPPER = np.array(self.upper_limits, dtype=np.float32)
        # if tool_id == 400006:
